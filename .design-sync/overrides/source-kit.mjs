...
// forked from design-sync lib/source-kit.mjs — kebab-case component dirs vs PascalCase names.
//
// Upstream strips the component's own directory from the group path with
// `s.toLowerCase() !== c.name.toLowerCase()`, which assumes the directory is spelled like the
// component ("Button/Button.tsx"). This repo's folder-per-component rule mandates kebab-case
// (`.claude/rules/component-architecture.md`), so "image-glow" never equals "imageglow", the own-dir
// segment survives, and `.at(-1)` returns it — giving every component a group of one and ~60
// single-item groups in the picker. Comparing on identifier letters alone recovers the repo's real
// taxonomy (effects, buttons, icons, typography, containers, …).
const identOnly = (s) => s.toLowerCase().replace(/[^a-z0-9]/g, '');
const sameIdent = (a, b) => identOnly(a) === identOnly(b);
    ...
    for (const c of components) {
      ...
      c.group = slug(
        slash(relative(srcRoot, dirname(hit)))
          .split('/')
          .filter((s) => s && !sameIdent(s, c.name) && !GENERIC_DIR.has(s.toLowerCase()))
          .at(-1)
