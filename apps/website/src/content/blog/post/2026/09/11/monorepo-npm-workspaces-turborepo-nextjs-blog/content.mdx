...
So I turned the whole thing into a monorepo: npm workspaces plus [Turborepo](https://turborepo.com), seven workspaces
(three apps and four packages), three of which are now published on npm. This post is the playbook. It follows the
real chronology, five acts and the pipeline they all converge on, with the gotchas landed exactly where they bit me,
because a few of them cost me a lot more time than they should have.

One note before we start. The migration ran through the agentic SDLC pipeline I described in
[the software engineer skills pyramid post](/blog/post/2026/07/10/software-engineer-skills-pyramid-harness-sdlc):
explore, plan, implement and review in a loop, then a pull request. That is why this is around 27 small pull requests
instead of one unreviewable "monorepo" commit.
...
- **[release-it](https://github.com/release-it/release-it)** with `@release-it/conventional-changelog` to cut
...
Three pull requests ([#534](https://github.com/chicio/chicio-blog/pull/534), [#535](https://github.com/chicio/chicio-blog/pull/535), and [#536](https://github.com/chicio/chicio-blog/pull/536)) landed while the repository was still one plain Next.js app. None of them
created a workspace. None of them moved a file into `packages/`. All they did was sever the design system from
everything around it:

- **[#534](https://github.com/chicio/chicio-blog/pull/534)** made it independent of the website: no imports from `lib/`, from `features/`, or from `content/`
- **[#535](https://github.com/chicio/chicio-blog/pull/535)** made it framework agnostic: no imports from `next` at all
- **[#536](https://github.com/chicio/chicio-blog/pull/536)** gave it its own stylesheet, instead of borrowing the app's global CSS
...
graph TD
    ...
    subgraph After["After: the dependency points one way only"]
        ...
        SITE2 --> BIND2
        BIND2 -->|"injected props"| DS2
    end
    Before ~~~ After
```

The interesting half is [#535](https://github.com/chicio/chicio-blog/pull/535), because "remove the `next` import" sounds trivial until you notice that a design system
...
PR [#537](https://github.com/chicio/chicio-blog/pull/537) is the one that turned the repository into a monorepo. The final layout, three apps and four packages:
...
The second gotcha is environment variables. Turborepo requires every variable a task reads to be declared, and
**an undeclared variable is silently stripped rather than erroring**. The site's secrets therefore live in `apps/website/turbo.json`, on the tasks that actually run the app:
...
PRs [#538](https://github.com/chicio/chicio-blog/pull/538) through [#546](https://github.com/chicio/chicio-blog/pull/546) did the actual extraction and put the result on npm. This is where "monorepo" stops being a
...
The repository has a script for this, `scripts/verify-packages.mjs`, and it runs both as its own CI job and again
before every publish. It does six things in order: it builds every package, runs `npm pack` on each one to produce the
real tarball, lints that tarball's published surface with `publint --strict`, checks its type resolution with
`@arethetypeswrong/cli`, installs the actual tarballs into a throwaway app in a temp directory, and imports them from
there exactly as a consumer would.

It's the piece I'd port to any repository that publishes from a workspace, and the reasoning is worth quoting from the
script's own header, because it's the whole argument:
...
The script closes that gap by doing exactly what a real consumer does, and the most important line in it is the peer
list:
...
PRs [#547](https://github.com/chicio/chicio-blog/pull/547) through [#550](https://github.com/chicio/chicio-blog/pull/550) gave the design system a public face. Two decisions here are worth stealing.
...
this arrangement free. The hub is live as [fabrizioduroni.it labs](https://chicio.github.io/chicio-blog/), with the
[design system Storybook](https://chicio.github.io/chicio-blog/design-system/) under `/design-system/` and, once the
next act lands, the [matrix rain showcase](https://chicio.github.io/chicio-blog/matrix-rain/) under `/matrix-rain/`.
It deploys independently of the site itself (the blog goes to Vercel, and the workflow only runs when the packages,
the showcases, or the hub change).

## Act 4: absorbing a second repository

PRs [#565](https://github.com/chicio/chicio-blog/pull/565), [#567](https://github.com/chicio/chicio-blog/pull/567), and [#568](https://github.com/chicio/chicio-blog/pull/568) brought `matrix-rain-webgpu` and its Astro documentation site in from their own repository.
...
governs it like everything else. It now sits on the hub as the
[matrix rain showcase](https://chicio.github.io/chicio-blog/matrix-rain/), next to the design system's Storybook. Its release line stays scoped to both directories at once, since the package and its
docs move together:
...
## CI: seven gates, one build, one e2e

Everything above converges on a single pipeline. Seven jobs run in parallel, all seven gate the build, and the build
gates the end to end suite:
...
permissions:
  contents: read
  id-token: write # exchanged for a short-lived Turborepo remote-cache token
...
jobs:
  ...
  build:
    name: Build
    runs-on: ubuntu-latest
    needs: [lint, format, knip, validate-architecture, typecheck, test, verify-packages]
    steps:
      - name: Checkout
...
Every job has the same preamble (checkout, setup Node, `npm ci`, authenticate the remote cache) and then calls exactly
one `npx turbo run <task>`. The jobs don't know how many workspaces exist, which tools each one uses, or in what order
things must build. Turbo knows, and it's the same command I run locally.

`format` is the one gate that doesn't go through turbo. It runs `npm run format:check`, which is prettier over the
whole repository, because prettier reads one root config and one root `.prettierignore`: scoping it per workspace
would only hide files from it. Getting that gate to a state where it could run at all was its own small project. It
reported 799 files, and the fix was two unrelated things. Some of those files prettier had no business walking (the
vendored agent tooling, and `matrix-rain-webgpu`, which is formatted by oxfmt and whose own `lint` script runs
`oxfmt --check`, so rewriting it would have turned lint red). The rest had genuinely never been formatted with this
config. There was also a trap worth knowing about: prettier reads the **root** `.gitignore` only, so build output
covered by a workspace's own `.gitignore` stayed visible to the gate, and it passed on a clean checkout while failing
after a build.

Three details in the workflow:

- `node-version-file: "package.json"` reads the `engines.node` field, so the Node version is declared once and
  followed everywhere
- remote caching authenticates with **OIDC**, not a stored token. Each job exchanges GitHub's OIDC token for a
  short-lived Turborepo token scoped to the team rather than to me, uses it, and revokes it when the job ends. Nothing
  lives in the repository as a secret, which is the same reasoning as the publishing workflow two acts ago
- that exchange step is `continue-on-error` in every job, deliberately. The action fails hard when it cannot get a
  token, so without it a missing policy, a fork, or any context where `id-token` is not granted would take all seven
  gates red. With it, turbo simply runs without a remote cache

That last point has a sharp edge worth stating, because it cost me a round of confusion. GitHub reports a
`continue-on-error` step as **success** even when it exited non-zero, so a green checkmark tells you nothing about
whether caching is actually on. The only honest signal is turbo's own log line, `• Remote caching enabled`. The first
run after wiring this up was green in all eight jobs while every single one of them silently failed the exchange,
because the `TURBO_TEAM` variable did not exist yet.

Once it did, the numbers were real. Every CI job is a fresh runner with no local cache, so every restored task is a
genuine remote hit: lint restored 2 of 6 tasks, typecheck 1 of 8, test 1 of 5, and build 3 of 6. And the build job is
where the honesty tax from Act 1 shows up one more time:

```
WARNING  failed to contact remote cache: HTTP status client error (400 Bad Request)
```

That is the website's 427 MB `.next` being refused by the artifact store, exactly as it is on Vercel. Turbo logs it as
a warning rather than an error, so the job still passes, and the small package artifacts around it still cache
normally.
...
- PR [#553](https://github.com/chicio/chicio-blog/pull/553) bumped the React group **across 4 directories with 5 updates**, in a single PR
- PR [#559](https://github.com/chicio/chicio-blog/pull/559) did the same for the testing group across 3 directories with 8 updates
- PR [#555](https://github.com/chicio/chicio-blog/pull/555) did it for the Next group across 2 directories
...
with many independent apps that can share a remote cache. Wiring that cache into CI later did help a little, as the
numbers in the previous section show, but those are seconds shaved off gates that already run in parallel, not off the
deploy that actually puts the site live. For one person and one site, the relevant levers turned out
...
- the [fabrizioduroni.it labs](https://chicio.github.io/chicio-blog/) hub, which collects both showcases
- the [design system Storybook](https://chicio.github.io/chicio-blog/design-system/), every component with its stories
- the [matrix rain showcase](https://chicio.github.io/chicio-blog/matrix-rain/), the WebGPU effect and its docs
