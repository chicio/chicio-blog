...
In this way, even if the final merge request results in a big amount of changes, we are still able to navigate the implementation commit by commit, checking what the LLM produced and whether it matches our expectations. Note that this is one of many points in the workflow where our experience as engineers is paramount: best practices now matter more than ever if we want to produce a well engineered (and so, maintainable) code.
...
- **dynamic workflows**: we want to test [dynamic workflows](https://code.claude.com/docs/en/workflows), the Claude Code feature that orchestrates subagents at scale through scripts, and understand where this kind of orchestration fits next to our deterministic pipelines, starting from their parallelizable phases
- **Claude Design integration**: we are defining our design system and our user flows in [Claude Design](https://claude.ai/design), so connecting it to `sdlc-frontend`, the pipeline that builds our frontend, feels like a natural evolution
