import { getValueFor, incrementValueByOneFor, setExpirationFor, setValueFor } from "../upstash/upstash-redis";
...
const isBelowThrottleOfSecondsFor = async (identifier: string): Promise<number | undefined> => {
    ...
    if (lastSubmission) {
        const secondsSinceLastSubmission = Math.floor((Date.now() - lastSubmission) / 1000);
...
const hasReachedFormSubmitLimit = async (identifier: string): Promise<boolean> => {
...
export const checkRateLimitFor = async (identifier: string): Promise<RateLimitResult> => {
    try {
