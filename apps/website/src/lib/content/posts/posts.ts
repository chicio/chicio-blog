...
export const groupArrayBy: <T>(array: T[], numberPerGroup: number) => T[][] = (data, n) => {
    const group = Array(0);
...
export const getPostsPaginationFor = (page: number): Pagination | undefined => {
    ...
    const previousPageUrl = page === 2 ? slugs.blog.home : `${slugs.blog.blogPostsPage}/${page - 1}`;
    ...
    return {
        ...
        nextPageUrl: postsPage.hasNext ? `${slugs.blog.blogPostsPage}/${page + 1}` : undefined,
...
export const aggregateAuthorsWithPosts = (posts: Content[]): AuthorSummary[] => {
    ...
    return [...summaries.values()].sort((a, b) => (a.author.name.toLowerCase() < b.author.name.toLowerCase() ? -1 : 1));
};
...
export const getAuthorWithPostsBySlug = (authorSlug: string): { author: Author; posts: Content[] } | undefined =>
    findAuthorWithPostsBySlug(posts.list(), authorSlug);

/**
 * READ NEXT
 */

export const rankReadNextPosts = (currentTags: string[], candidates: Content[], limit: number): Content[] => {
    const scored = candidates.map((post) => {
        const sharedTagCount = post.frontmatter.tags.filter((tag) => currentTags.includes(tag)).length;
...
export const getReadNextPosts = (currentSlug: string, limit = 2): Content[] => {
    const allPosts = posts.list();
    const currentPost = allPosts.find((post) => post.slug.formatted === currentSlug);
    const candidates = allPosts.filter((post) => post.slug.formatted !== currentSlug);
