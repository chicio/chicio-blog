...
export const ConsoleTimeInformation: FC<ConsoleTimeInformationProps> = ({
    ...
    return (
        <div className={`flex flex-col gap-2 md:flex-row ${className}`}>
            ...
            {acquiredYear && <InfoPill icon={<BiCart />} label="Acquired" value={acquiredYear} />}
            {bits && <InfoPill icon={<IoGameControllerOutline />} label="Architecture" value={bits} />}
            {generation && <InfoPill icon={<TbDeviceGamepad2 />} label="Generation" value={generation} />}
        </div>
    );
};
