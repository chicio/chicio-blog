...
|---------|--------------|---------------|
| **Size** | Fixed at compile-time | Flexible, resizes at runtime |
| **Memory** | Allocated once | Can grow/shrink, may require reallocation |
| **Efficiency** | Fast access, no resizing cost | Amortized O(1) insertion, but resizing costs O(n) |
| **Flexibility** | Rigid, not suited for unknown sizes | Very flexible, suited for dynamic data |
| **Languages** | C, C++ | Python, Java, Swift, Kotlin, TypeScript |

**Key Insight:** Static arrays are better when the size is known and fixed; dynamic arrays excel when flexibility is needed.
