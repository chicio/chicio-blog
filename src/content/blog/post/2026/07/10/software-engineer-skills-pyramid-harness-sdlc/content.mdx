...
To be honest with you, at the moment this last part is fully implemented only in the mobile pipeline: `sdlc-frontend` carries the QA evidence to the merge request gate, but it does not post the comment yet (it is on our todo list).
...
- REMINDER (added during review): mention that sdlc-frontend must catch up with sdlc-mobile on posting the QA report + screenshots as an MR comment.
