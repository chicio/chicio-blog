...
As you can see in the diagram below, the flow starts with a synchronous regex-based prompt injection check (instant and free). If that passes, two LLM-based checks run in parallel via `Promise.all`: a Llama Guard content safety classification and an LLM-as-judge topic relevance check. Only when all three checks pass does the message reach the main model.
...
graph TD
    ...
    D --> E{"Llama Guard 3 8B\ncontent safety"}
    D --> F{"Llama 3.1 8B\ntopic relevance"}
    E -- "Unsafe" --> G["Reject: safety\nviolation message"]
    E -- "Safe" --> H{"Both passed?"}
    F -- "Off-topic" --> I["Reject: off-topic\nredirect message"]
    F -- "On-topic" --> H
    H -- "Yes" --> J["Main Llama 3.3 70B\nmodel + RAG"]
```

The whole pipeline adds minimal overhead because Groq's inference is extremely fast, and the two LLM checks use small (8B parameter) models.
