...
### CLDR distance matching in action

Now that we understand the CLDR/ICU language distance algorithm, let's see it in action with some concrete scenarios
that really highlight how it works. These examples are particularly important because they demonstrate cases that
would be impossible to explain with a simple "strip and compare" fallback model.

**Scenario: Device set to `en` (no region)**

When the device language is set to "English" (without any dialect) and the region is set to "United States", the
device locale is `en-US`. With CLDR likely subtags, `en` expands to `en-Latn-US`. Since our app has an `en`
localization (which is exactly `en-US` after expansion), this is an exact match with distance 0.
`Bundle.preferredLocalization` returns `en` and the app shows "Hello STANDARD!".

![Device set to English generic, matching the en localization](/images/posts/which-locale-en-generic-standard.jpg)

**Scenario: Device set to `en-IN` (with `en-IN` in the app)**

When the device language is set to "English (India)", the locale is `en-IN`. Since our app includes an `en-IN`
localization, this is an exact match (distance 0). `Bundle.preferredLocalization` returns `en-IN` and the app
shows "Hello India!". Exact matches always have the highest priority, no distance calculation needed.

![Device set to English India, exact match with en-IN localization](/images/posts/which-locale-en-in-with-en-in.jpg)

**Scenario: Device set to `en-IN` (without `en-IN` in the app)**

This is the scenario that surprises most developers. If we remove the `en-IN` localization from the app (you can do
this in Xcode via Project -> Info -> Localizations, then delete the "English (India)" entry and recompile), something
unexpected happens.

With `en-IN` as the device locale and the app supporting `en`, `en-GB`, and `en-IE` (among others), you might
expect iOS to pick the generic `en`. After all, "English India" is still "English", right? But instead,
`Bundle.preferredLocalization` returns `en-GB`! Here's why:

```
en-IN (desired) vs en-GB (supported):
  language:  en → en  = 0
  script:    Latn → Latn = 0
  region:    IN → GB  = 4  (special CLDR rule: en_*_GB has distance 4 from other EN variants)
  TOTAL = 4  ← WINS

en-IN (desired) vs en [= en-US after likely subtags] (supported):
  language:  en → en  = 0
  script:    Latn → Latn = 0
  region:    IN → US  = 6  (special CLDR rule: en_*_US has distance 6 from other EN variants)
  TOTAL = 6
```

British English wins because CLDR knows that Indian English is linguistically closer to British English than to
American English. This makes perfect sense historically: India was a British colony and Indian English inherits
British spelling, vocabulary, and conventions. The algorithm reflects this real-world linguistic relationship.

![Device set to English India without en-IN localization, falls back to en-GB](/images/posts/which-locale-en-in-without-en-in.jpg)

This scenario is a perfect demonstration of why understanding CLDR/ICU distance matching is essential for fully
grasping how iOS locale selection works. The behavior is not random and it's not a simple fallback. It's a
carefully designed algorithm backed by real linguistic data.
