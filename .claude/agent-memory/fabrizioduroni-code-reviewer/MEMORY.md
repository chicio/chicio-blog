...
- [next-build-injects-claude-md-block.md](next-build-injects-claude-md-block.md) — your own `npm run build` dirties CLAUDE.md with a nextjs-agent-rules block; not the implementer's diff, restore it before finishing
- [fact-sheet-drift-intent-and-authorship.md](fact-sheet-drift-intent-and-authorship.md) — fact-sheet-bounded prose drifts as widened authorship ("chime and startup screen were built by X") and manufactured intent ("a deliberate contrast to..."), not invented names
- [verify-embed-titles-against-oembed.md](verify-embed-titles-against-oembed.md) — check a new `<Youtube title>` against the video's own oembed title AND the prose order; a symptom bulleted after the embed (with its own screenshot) is not in the video
- [prove-e2e-guard-non-vacuous-with-control-page.md](prove-e2e-guard-non-vacuous-with-control-page.md) — prove a new e2e assertion fails on regression by running its locators from a scratchpad config against a page lacking the feature (NES page has no Startup section)
- [always-mounted-overlay-stale-hook-state.md](always-mounted-overlay-stale-hook-state.md) — overlays that stay mounted and `return null` keep hook state; useTypewriter-style per-open animations run only the FIRST open — open it twice live
- [deleted-ui-leaves-stale-mdx-instructions.md](deleted-ui-leaves-stale-mdx-instructions.md) — deleting an interaction leaves src/content MDX telling users to do it; verify via .next/server/app/markdown/&lt;slug&gt;.body
- [early-mounted-autoplay-video-loses-its-head.md](early-mounted-autoplay-video-loses-its-head.md) — a hidden-but-autoplaying video keeps playing; measure currentTime at the first VISIBLE frame, not after — opacity-hiding never pauses media
