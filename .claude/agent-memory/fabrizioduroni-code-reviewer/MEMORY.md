...
- [derived-anchor-ids-must-be-diffed-against-rendered-output.md](derived-anchor-ids-must-be-diffed-against-rendered-output.md) — lib-derived heading slugs must be diffed against built HTML for every content.mdx; rehype-slug does NOT trim, so a helper's `.trim()` silently kills anchors
- [real-content-degenerate-shapes-must-drive-hierarchy-tests.md](real-content-degenerate-shapes-must-drive-hierarchy-tests.md) — replay heading-tree/grouping logic over the real archive: h3-only and h4-only blog posts break naive "nest under last group" code
- [intersection-observer-stub-hides-scroll-spy.md](intersection-observer-stub-hides-scroll-spy.md) — a no-op IntersectionObserver stub makes scroll-spy look covered while never running; demand the use-in-view.test.tsx capture-the-callback pattern
- [run-test-coverage-not-just-test-run.md](run-test-coverage-not-just-test-run.md) — CI's test job is `test:coverage`; build-cache is production-only so content-walking tests re-parse all 547 files and sit near the 5s testTimeout under v8
- [type-only-import-needs-types-package.md](type-only-import-needs-types-package.md) — a dep added for a type-only import must be `@types/x`, not `x`; cat node_modules/<x>/package.json and diff npm audit against main
- [in-flow-layout-mutation-breaks-smooth-scroll.md](in-flow-layout-mutation-breaks-smooth-scroll.md) — scroll-spy state committed mid-scroll can abort it; judge the fix, not the stated mechanism, and probe the xl rail at 1800px (e2e runs at 1280px)
- [per-item-ingestion-cost-times-out-first-test.md](per-item-ingestion-cost-times-out-first-test.md) — new per-content-item ingestion work must be timed against the FIRST it() of every content-walking node test; a memo + one file's beforeAll warm-up fixes only the observed instance
- [tailwind-v4-emits-unprefixed-twin-of-arbitrary-variant.md](tailwind-v4-emits-unprefixed-twin-of-arbitrary-variant.md) — `xl:left-[calc(...)]` also emits an unprefixed base rule; that dead twin is v4 engine behavior, verify in .next before raising it
