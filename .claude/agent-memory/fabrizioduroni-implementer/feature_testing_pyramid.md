...
- Every real MDX heading is wrapped in its own `.heading-anchor` `<a>` (rehype-autolink-headings, `next.config.ts`) whose accessible name is the heading text WITH a leading "# " prefix (a CSS `::before` glyph, part of the accname computation). Any OTHER link-shaped UI whose name overlaps a heading's text (e.g. a table-of-contents entry) is ambiguous against it in a page-wide `getByRole("link", {name})` query — scope through the specific landmark (`page.getByRole("navigation", {name: "..."}).getByRole(...)`), and don't anchor heading-name regexes to `^` (they won't match the "# " prefix).
- `toBeHidden()`/`isVisible()` does NOT recognize an ancestor's `overflow-hidden` + animated `height:0` (e.g. a Framer Motion collapsed-accordion panel) as hiding a descendant — it only inspects the element's own box, not ancestor clipping. Assert disclosure state via the trigger's `aria-expanded` attribute instead. A native closed `<details>` IS correctly recognized as hidden (different code path, real UA suppression).
- An in-flight `scrollIntoView({behavior: "smooth"})` can be shortened if something ELSE mutates layout above the target while it's animating (observed: scroll-spy reactively expanding a sibling accordion mid-scroll). If a feature's own reactive state can shift layout during navigation, force deterministic instant scrolling in the affected e2e test via whatever mechanism the app's OWN motion toggle reads (check for a `localStorage`-backed toggle before reaching for `page.emulateMedia({reducedMotion})` — that only affects the OS media query, which a hook may not consult at all).
